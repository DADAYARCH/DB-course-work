...
def write_create_sql(table: Table) -> None:
    with open(f"step3\sql\{table.name}.schema.sql", "w") as f:
...
def generate_data_sql(table: Table, cnt: int) -> None:
    ...
    with open(f"step3\sql\{table.name}.data.sql", "w") as f:
        records_cnt = 0
        for i in range(cnt):
...
if __name__ == "__main__":
    ...
    generate_all_data_sql(100, [MAP_TABLE, VISITED_TABLE, COMMENT_TABLE, PHOTO_TABLE, FRIENDS_TABLE])
